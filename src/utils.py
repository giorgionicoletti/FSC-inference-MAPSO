...
@nb.njit
def nb_simulate_POMDP(nb_generate_state_transition, 
                   nb_generate_observation,
                   nb_policy_function, nb_reward_function,
    ...
    observations = np.empty(num_steps, dtype=np.int64)
    states = np.empty(num_steps, dtype=np.int64)
    rewards = np.empty(num_steps, dtype=np.float64)

    beliefs[0] = initial_belief
    states[0] = initial_state

    for t in range(num_steps):
        actions[t] = nb_policy_function(beliefs[t], policy_params)
        new_state = nb_generate_state_transition([states[t]], actions[t], env_params)
        observations[t] = nb_generate_observation(new_state, actions[t], [states[t]], env_params)

        rewards[t] = nb_reward_function(observations[t], actions[t], states[t], reward_params)
        if t < num_steps - 1:
            beliefs[t + 1] = nb_belief_update_function(actions[t], observations[t], beliefs[t], env_params)
            states[t + 1] = new_state[0]

    return beliefs, actions, observations, states, rewards

@nb.njit(parallel = True)
def nb_generate_POMDP_trajectories(nb_generate_state_transition, 
                                   nb_generate_observation,
                                   nb_policy_function, nb_reward_function,
                                   nb_belief_update_function,
    ...
    states = np.empty((NRep, NSteps), dtype=np.int64)
    rewards = np.empty((NRep, NSteps), dtype=np.float64)

    for i in nb.prange(NRep):
        beliefs[i], actions[i], observations[i], states[i], rewards[i] = nb_simulate_POMDP(
            nb_generate_state_transition,
            nb_generate_observation,
            nb_policy_function,
            nb_reward_function,
            nb_belief_update_function,
            initial_beliefs[i], NSteps, initial_states[i], env_params,
            reward_params, policy_params
        )

    return beliefs, actions, observations, states, rewards

def generate_POMDP_trajectories(env, nb_policy_function, nb_reward_function,
                                nb_belief_update_function, NRep, NSteps,
                                initial_beliefs, initial_states,
    ...
    for idx_trj in range(NRep):
        current_actions = []
        current_observations = []
        current_states = []
        for idx_step in range(NSteps):
            if ObsSpace is not None:
                current_observations.append(ObsSpace[observations[idx_trj][idx_step]])
            else:
                current_observations.append(observations[idx_trj][idx_step])
            
            if StatesSpace is not None:
                current_states.append(StatesSpace[states[idx_trj][idx_step]])
            else:
                current_states.append(states[idx_trj][idx_step])
        for idx_step in range(NSteps):
            if ActSpace is not None:
                current_actions.append(ActSpace[actions[idx_trj][idx_step]])
            else:
                current_actions.append(actions[idx_trj][idx_step])

        named_actions.append(current_actions)
        named_observations.append(current_observations)
        named_states.append(current_states)

        
    actions = named_actions
    observations = named_observations
    states = named_states
...
def plot_tetrahedron_simplex(probabilities, colors, cmap,labels=None, title="4D Simplex in Tetrahedron",
                             s = 50):
    """
    Plot 4 probabilities that sum to 1 in a 3D tetrahedron.
    
    Parameters:
    - probabilities: array of shape (n_points, 4) where each row sums to 1
    - labels: list of 4 strings for the vertex labels
    - title: plot title
    """
    
    # Define tetrahedron vertices in 3D with one face in the xy plane
    # Place three vertices in the xy plane (z=0) forming an equilateral triangle
    # and the fourth vertex above the center of this triangle
    h = np.sqrt(2/3)  # height of tetrahedron above the base triangle
    vertices = np.array([
        [1, 0, 0],              # vertex 0 - on xy plane
        [-0.5, np.sqrt(3)/2, 0], # vertex 1 - on xy plane  
        [-0.5, -np.sqrt(3)/2, 0], # vertex 2 - on xy plane
        [0, 0, h]               # vertex 3 - above the triangle
    ])
    
    # Convert probabilities to 3D coordinates
    # Each point is a weighted sum of the tetrahedron vertices
    points_3d = probabilities @ vertices
    ...
    for edge in edges:
        v1, v2 = edge
        ax.plot3D(*zip(vertices[v1], vertices[v2]), 'k-', alpha=0.3, linewidth=1)
        
    # Plot the vertices
    ax.scatter(*vertices.T, s=100, c='k', alpha=0.8)
    ...
    for i, (vertex, label) in enumerate(zip(vertices, labels)):
        ax.text(vertex[0]*1.1, vertex[1]*1.1, vertex[2]*1.1, label, 
    ...
    ax.scatter(*points_3d.T, c=colors, s=s, alpha=0.7, cmap = cmap)
    
    # If there's a trajectory, connect the points
    # if len(points_3d) > 1:
    #     ax.plot(*points_3d.T, 'gray', ls = "--", alpha=0.5, linewidth=1)
...
def plot_nu_evolution(nu_mat, memories, cmap, s = 50, start_t=None, end_t=None):
    """
    Plot the evolution of probabilities over time for a specific action-observation pair.
    
    Parameters:
    - nu_mat: your probability matrix (n_actions, n_observations, n_timepoints)
    - action_idx: which action to plot
    - obs_idx: which observation to plot  
    - start_t, end_t: time range to plot (optional)
    """
    
    if start_t is None:
        start_t = 0
    if end_t is None:
        end_t = nu_mat.shape[2]
    
    probs_matrix = np.zeros((end_t - start_t, 4))
    probs_matrix[:, 0] = nu_mat[0, 0, start_t:end_t]  # action 0, obs 0
    probs_matrix[:, 1] = nu_mat[0, 1, start_t:end_t]  # action 0, obs 1  
    probs_matrix[:, 2] = nu_mat[1, 0, start_t:end_t]  # action 1, obs 0
    probs_matrix[:, 3] = nu_mat[1, 1, start_t:end_t]  # action 1, obs 1
    ...
    # Plot
    fig, ax = plot_tetrahedron_simplex(
        probs_matrix,
        colors = memories,
        cmap = cmap,
        s = s,
        labels=['AR|O0', 'AR|O1', 'AL|O0', 'AL|O1'],
        title=f"Probability Evolution Over Time (t={start_t} to {end_t-1})"
    )
